...
func (a *API) createScaler(w http.ResponseWriter, req *http.Request) {
	...
	resp, _ := a.etcdcli.DoGet(req.Context(), path, etcdv3.WithCountOnly())
	if resp.Count == 0 {
	...
	v, _ = json.Marshal(&s)
	_, err := a.etcdcli.DoPut(req.Context(), path, string(v))
	if err != nil {
...
func (a *API) deleteScaler(w http.ResponseWriter, req *http.Request) {
	...
	resp, err := a.etcdcli.DoDelete(req.Context(), path, etcdv3.WithPrefix())
	if err != nil {
		a.respondError(w, apiError{
			code: http.StatusInternalServerError,
			err:  err,
